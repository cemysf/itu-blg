...
static int addtag(const char * filename, char * tag_v, char * tag_type){
	IptcData * d;
	FILE * infile, *outfile;
	...
	iptc_dataset_set_data (ds, tag_v, strlen(tag_v), IPTC_DONT_VALIDATE);
...
{   
    ...
    if (strcmp(path, "/") == 0) {
		...
		else if(strcmp(directory_ks,"subjects")==0){
			for (i=0; i < d->count; i++) {
				e = d->datasets[i];
				if(e->record == 2 && e->tag == 12){
					iptc_dataset_get_data (e, buffer, sizeof(buffer));
...
static int psk_mknod(const char *path, mode_t mode, dev_t dev){
	char tag_type[256];
	char tag_value[256];
	char directory[PATH_MAX];
	findDirectory(tag_type,path);
	if(strcmp(tag_type,"keywords")==0){
		strcpy(tag_type, "Keywords");
	}
	if(strcmp(tag_type,"subjects")==0){
		strcpy(tag_type, "SubjectRef");
	}
	findTag(tag_value, path);
	int index = findFileName(path);
	file_pathTranslation(path+index,directory);
	addtag(directory,tag_value,tag_type);
	return 0;
}
