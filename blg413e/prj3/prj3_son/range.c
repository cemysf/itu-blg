...
struct range_dev {
    ...
    struct cdev cdev;
    int counter, step, start, end, index, rotate, eof_mode, Is_eof, device_no;
};
                   ...
                   loff_t *f_pos)
{
    ...
    if(!dev->rotate){
		if(dev->counter + dev->step > dev->end || dev->counter + dev->step < dev->start){
			if(dev->eof_mode){
				dev->Is_eof = 1;
				goto eof;
			}
		...
		else dev->counter = next_number(dev->counter, dev->start, dev->end, dev->step);
		dev->index++;
	}
	else{
		if(dev->counter - dev->step < dev->start || dev->counter - dev->step > dev->end){
			if(dev->eof_mode){
				dev->Is_eof = 1;
				goto eof;
			}
...
int range_init_module(void)
{
    ...
    if (range_major) {
        ...
        result = alloc_chrdev_region(&devno, range_minor, range_nr_devs,	
                                     "range");		// register a range of char device numbers 
    ...
    for (i = 0; i < range_nr_devs; i++) {
        ...
        err = cdev_add(&dev->cdev, devno, 1);		//adds the device
        if (err)
